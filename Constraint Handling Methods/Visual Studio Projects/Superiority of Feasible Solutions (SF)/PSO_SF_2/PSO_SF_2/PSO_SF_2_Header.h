...
#define Vmax 0.75 //速度极值，如果取值过大，不利于个体的优化,一般是upbound-lowbound
#define  r 0.1
...
//	 q_value[1]=(g[1]>=0?g[1]:0);
 ...
 double *g_function(double *p)//constraints
 {
	 ...
	 g_value[0]=100-(p[0]-5)*(p[0]-5)-(p[1]-5)*(p[1]-5);
	 g_value[1]=(p[0]-6)*(p[0]-6)+(p[1]-5)*(p[1]-5)-82.81;
	 return	g_value;
 }
 double xita_function(double q)//assignment function
 {
 ...
 bool Is_feasible(double *p)//To judge whether the constraints are violated
 {
	 ...
	 if ((constraint[0])<=1000&&constraint[1]<=1000)
	 {
		 delete[] constraint;
		 return true;
	 }
