...
bool Is_feasible(double *p);//To judge whether the constraints are violated
 ...
 double f(double *p)   //Original objective function
 {
	 double f_value;
	 f_value=(p[0]-2)*(p[0]-2)+(p[1]-1)*(p[1]-1);
	 return f_value;

 }
 double h(int k)	//Penalty value
 {
	 double h_value;
	 h_value=sqrt(k);
	 return h_value;
 ...
 double H(double *p)//Penalty factor
 {
	  int i;
...
//	 q_value[1]=(g[1]>=0?g[1]:0);
 ...
 bool Is_feasible(double *p)//To judge whether the constraints are violated
 {
	 ...
	 if (abs(constraint[0])<=0.0001&&constraint[1]<=0.0001)
	 {
		 delete[] constraint;
		 return true;
	 }
...
void ParticleSwarm::init(){
	...
	lowBound[0]=-2,upperBound[0]=2;
	lowBound[1]=-1,upperBound[1]=1;
	...
	{   
		...
		x[0]=2*x[1]-1; //to take advantage the active constraint
