...
int main(int argc, char* argv[])
{
	...
	PSOData = fopen("PSO_SF_1.dat","w+");//to clear the file and write into new data。 
	if (PSOData == NULL)
	{
	...
	PSO_EXCEL = fopen("PSO_SF_1_EXCEL.dat","w+");//to clear the file and write into new data。 
	if (PSO_EXCEL == NULL)
	{
	...
	else
	{
		for (test_number=0;test_number<TESTTIMES;test_number++)
		{
			...
			for (i = 0; i < RUNTIMES; i++)
			{
				...
				} while (p.num_feasible<r*PSO_popsize);
