...
int main(int argc, char* argv[])
{
	...
	PSOData = fopen("PSO_PFM_T4.dat","w+");//clear the file and write new data into it。 
	if (PSOData == NULL)
	{
		printf("The file is not exit!\n");
		exit(1);
	}
	PSO_EXCEL = fopen("PSO_PFM_T4_EXCEL.dat","w+");//clear the file and write new data into it。 
	if (PSOData == NULL)
	{
		printf("The file PSO_PFM_T4_EXCEL is not exit!\n");
		exit(1);
	}
	else
	{
