...
//	 q_value[1]=(g[1]>=0?g[1]:0);
 ...
 bool Is_feasible(double *p)//To judge whether the constraints are violated
 {
	 ...
	 constraint[0]=p[0]-2*p[1]+1;
	 constraint[1]=(p[0]*p[0]/4+p[1]*p[1]-1);
	 if (abs(constraint[0])<0.01&&constraint[1]<0.01)
	 {
		 return true;
	 }
	 else
...
void ParticleSwarm::init(){
	...
	lowBound[0]=-10,upperBound[0]=10;
	lowBound[1]=-10,upperBound[1]=10;
